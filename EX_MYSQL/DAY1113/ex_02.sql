...
insert into tbl_product values(1, "TV", 1000);  -- 1개 추가
...
insert into tbl_product(price) values (9990); -- pid, pname 필드/컬럼은 null 입력 가능


## => 데이터 조회 : (1) 모든 컬럼 조회
## 	  select 조회할 컬럼명 from 테이블이름;
select pid, pname, price from tbl_product;
select * from tbl_product;					-- * 의미 : 테이블에 모든 컬럼alter
